...
const float WorkspaceCable::CABLE_CURVEPOINT_OFFSET = 50.0f;

WorkspaceCable::WorkspaceCable(WorkspaceNode *Input, WorkspaceNode *Output, int FromIndex, int ToIndex)
: WorkspaceItem(WorkspaceItemTypeCable, wxRect(0, 0, 0, 0)), 
	Input(Input), Output(Output), FromIndex(FromIndex), ToIndex(ToIndex)
{
	ConnectCable(Input, Output, FromIndex, ToIndex);
}

bool WorkspaceCable::IntersectsWith(float Left, float Top, float Right, float Bottom) const
{
...
bool WorkspaceCable::IntersectsWith(const wxRect& Rect) const
{
	if (!WorkspaceItem::IntersectsWith(Rect))
		return false;
...
void WorkspaceCable::ConnectCable(const WorkspaceNode *Input, const WorkspaceNode *Output, int FromIndex, int ToIndex)
{
	this->Input = Input;
	this->Output = Output;
...
bool WorkspaceCable::CanConnectFloatingEndTo(const ConnectorInfo& ConnectInfo)
{
	if (!Output)
...
void WorkspaceCable::ConnectFloatingEnd(const ConnectorInfo& ConnectInfo)
{
	if (!Output)
	{
		Output = (WorkspaceNode*)ConnectInfo.Owner;
		FromIndex = ConnectInfo.PortIndex;
	}
	else
	{
		Input = (WorkspaceNode*)ConnectInfo.Owner;
...
void WorkspaceCable::SetFloatingPosition(const wxPoint& NewFloatingPosition)
{
...
bool WorkspaceCable::ShouldDeleteOnSelectionDeletion() const
{
	if (WorkspaceItem::ShouldDeleteOnSelectionDeletion())
		return true;
...
void WorkspaceCable::ReactToItemsMoved(std::vector<WorkspaceItem*> ItemsThatWereMoved)
{
